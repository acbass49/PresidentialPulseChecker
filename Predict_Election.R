...
#adjusts score to be out of 100. Gets rid of unsure and third party categories.
weight_to_100 <- function(data){
  weight <- 100/sum(data[["pct"]])
  data <- data %>% filter(answer == "Trump")
  return(data[["pct"]]*weight)
}
...
ggplot()+
  ...
  geom_rect(data = extra_map, 
            ...
            inherit.aes = F)+
  ggtitle("Probabilities Of Trump Win By State")+
  theme_void()

ggplotly(p) %>% config(displayModeBar = F)

#upload to plotly online
Sys.setenv("plotly_username"="acbass49")
Sys.setenv("plotly_api_key"="3VdXsCXvpvzcq44wVU2u")
